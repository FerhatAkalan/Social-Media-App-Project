...
export const createVerificationRequest = async (username, requestData) => {
  try {
    const response = await axios.post(
      `/api/1.0/verifications/create/${username}`,
      requestData,
      {
        headers: {
...
export const checkLikeStatus = async (postId, username) => {
  try {
    const response = await axios.get(
      `/api/1.0/likes/check?postId=${postId}&username=${username}`
    );
...
export const fetchLikedUsers = async (postId) => {
  const response = await axios.get(`/api/1.0/likes/${postId}/liked-users`);
  return response.data;
};

export const followUser = (followerUsername, followedUsername) => {
  return axios.post(
    `/api/1.0/users/${followerUsername}/follow/${followedUsername}`
  );
};

export const unfollowUser = (followerUsername, followedUsername) => {
  return axios.post(
    `/api/1.0/users/${followerUsername}/unfollow/${followedUsername}`
  );
};

export const getFollowing = (username, page = 0, size = 10) => {
  return axios.get(
    `/api/1.0/users/${username}/following?page=${page}&size=${size}`
  );
};

export const getFollowers = (username, page = 0, size = 10) => {
  return axios.get(
    `/api/1.0/users/${username}/followers?page=${page}&size=${size}`
  );
};

export const checkFollowStatusApi = (followerUsername, followedUsername) => {
  try {
    const response = axios.get(
      `/api/1.0/users/${followerUsername}/is-following/${followedUsername}`
    );
    return response;
  } catch (error) {
